...
class User extends CI_Controller{
  ...
  public function index(){
		...
		$this->load->view('tamplate/header_user2', $data);
	...
	public function pemesanan(){
			...
			if ($this->form_validation->run() != true) {
					...
					$this->M_abtb->insert_data($data, 'transaksi');
					$this->M_abtb->delete_data(['id_pesan' => $data['id_pesan']], 'pemesanan');
	...
	public function bayarbooking($id){
			$data['judul'] = 'Payment';
			$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
			$data['pemesanan'] = $this->M_abtb->edit_data(['email' => $this->session->userdata('email'), 'id_pesan' => $id], 'transaksi')->row_array();
